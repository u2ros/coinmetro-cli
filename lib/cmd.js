...
module.exports.list = function (api) {
  return new Promise((resolve) => {
    let cmds = utils.readObj(cmdPath)
...
module.exports.run = function (api, name) {
  return new Promise((resolve, reject) => {
    ...
    cmds.forEach(cmd => {
      ...
      //need to remove these as dotenv does not override existing env variables (process.env takes precedence .env)
      delete env.demo
      delete env.token
      delete env.userId

      child.spawnSync(exe, args, { shell: true, stdio: 'inherit', env })
    })
