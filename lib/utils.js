    ...
    return new Promise((resolve, reject) => {
      ...
      .then(orders => {
        ...
        orders = orders.filter(o => {
          if (platform !== api.platform.margin) { // trade platform
            return o.margin ? false : true
          } else { //margin and tram
            if (o.tramID) {
...
function generateHistoryFunction (platform) {
  return function (api, pair='', kind = api.history.filled, since=null) {
    return new Promise((reject, resolve) => {
      ...
      .then(history => {
        history.sort((a, b) => { // TODO, additional sort by price
        ...
        history.forEach(h => {
          ...
          history = history.filter(h => {
            // TODO
            //
            if (platform !== api.platform.margin && !h.margin) return true
            if (platform === api.platform.margin && !h.tramID) return true
            if (platform === api.platform.tram) {
        ...
        printOrders(api, toShow, platform)
      })
      .catch(err => {
        reject(err)
      })
