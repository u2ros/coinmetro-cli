    ...
    if (typeof amount !== 'undefined' && typeof currency !== 'undefined') {
      ...
      .then(balances => {
        ...
        Object.keys(balances.available).forEach(k => {
          let balance = balances.available[k]
          if (typeof balances.collateral[k] !== 'undefined') {
            balance = balance - balances.collateral[k]
          }
          balance = balance.toFixed(4).padStart(13)
          console.log(`${balance}${k}`)
        })
        console.log('\nCollateral:')
        console.log('-'.repeat(70))
        Object.keys(balances.collateral).forEach(k => {
          let balance = balances.collateral[k].toFixed(4).padStart(13)
          console.log(`${balance}${k}`)
        })
        console.log('\nReserved (unmatched positions):')
        console.log('-'.repeat(70))
        Object.keys(balances.reserved).forEach(k => {
          let balance = balances.reserved[k].toFixed(4).padStart(13)
          console.log(`${balance}${k}`)
        })
        console.log('\nExposure (matched positions):')
        console.log('-'.repeat(70))
        Object.keys(balances.exposure).forEach(k => {
          let balance = balances.exposure[k].toFixed(4).padStart(13)
          console.log(`${balance}${k}`)
        })
        resolve()
      })
...
module.exports.default = module.exports.balance

module.exports.open = utils.open(constants.platform.margin)
