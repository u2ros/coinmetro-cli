...
module.exports.trades = function (api, pair, date, time='0:00') {
  ...
  pair = pair.toUpperCase()
  date = typeof date !== 'undefined' ? utils.parseDate(date, time) : Date.now() - 86400000
...
module.exports.chart = function (api, pair, chartconf) {
  ...
  return new Promise((resolve, reject) => {
    ...
    api.getHistoricalPrices(pair, api.chart[chartconf])
    .then(historicalPrices => {
      ...
      console.log(`${api.chart[chartconf].label} chart for ${c.bold.yellow(pair)}`)
...
module.exports.chart.help = {
  ...
  format: `api market chart ${c.bold.red('<pair>')} [${c.bold.cyan('<timeframe d|w|m|y>')}]`,
  examples: [
    `api market chart ${c.bold.red('xapieur')} ${c.bold.cyan('y')}`
  ]
}
