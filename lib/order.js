...
module.exports.list = function (api, pair, product=api.product.ex) {
...
module.exports.list.help = {
  ...
  format: `cm order open [${c.bold.red('<product ex|tram>')} ${c.bold.cyan('<pair>')}]`,
  examples: [
    `cm order open ${c.bold.red('ex')} ${c.bold.cyan('xcmeur')}`
  ]
}
