...
module.exports.run = function (api, name, ...args) {
  return new Promise((resolve, reject) => {
    let cmds = utils.readObj(cmdFile)
    ...
    for (let i=0; i<args.length; i++) {
      let idx = cmd.indexOf('%p')
      if (idx < 0) throw `No argument placeholder '%p' found in ${name} command`
      cmd = cmd.replace('%p', args[i])
    }
...
module.exports.run.help = {
  ...
  format: `cm cmd run ${c.bold.red('<name>')} [${c.bold.cyan('<0 or more args>')}]`,
  examples: [
    `cm cmd run ${c.bold.red('mycmd1')}`,
    `cm cmd run ${c.bold.red('showbook')} ${c.bold.cyan('30')}`,
  ]
}
