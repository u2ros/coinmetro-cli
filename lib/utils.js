...
function printOrders (api, orders) {
  orders.forEach(o => {
    ...
    if (pair === p1) {
      console.log(`${c.bold.green('B')} ${bq.padStart(10)}${bc.padEnd(3)} ${c.yellowBright('@' + p)} for ${sq.padStart(12)}${sc} ${f}% ${tif} ${c.dim(date)} ${c.dim(o.orderID)}`)
    } else {
      console.log(`${c.bold.red  ('S')} ${sq.padStart(10)}${sc.padEnd(3)} ${c.yellowBright('@' + p)} for ${bq.padStart(12)}${bc} ${f}% ${tif} ${c.dim(date)} ${c.dim(o.orderID)}`)
    }
  })
}
